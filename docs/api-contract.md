...
- `GET /api/reports/:runId/export?format=csv|xlsx|json|pdf` — binary download (use axios `responseType: 'blob'`, create object URL).
...
7. `/workflows/:id/run` — outbound config (API endpoint URL field, PDF/Excel export toggle, email field — stored but not sent in this build), "Run Reconciliation" → animated step progress (Ingest ✓ → Map ✓ → Match ⏳ → Score Breaks → Done), result banner (match rate, counts), "Download Report" (xlsx).
